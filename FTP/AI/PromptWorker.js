...
/**
 * Gets the components from an AI buffer png image.
 * @param {Buffer | String} buffer The AI image to check.
 * @returns {{CFGScale: String, Sampler: String, Seed: String, Size: String, Steps: String, parameters: String}}The components.
 */
function GetComponentsFromBuffer(buffer) {
...
function GetComponents(fileToRead) {
    if (ComponentCache[fileToRead] != null) return ComponentCache[fileToRead];
        ...
        const response = GetComponentsFromBuffer(OGdata);
...
module.exports = {
    GetComponents, PreloadPrompts, GetComponentsFromBuffer
}